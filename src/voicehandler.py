...
class VoiceExtended():
    def __init__(self, message):
        if message.reply_to_message:
            self.Voice_object = message.reply_to_message.voice
            self.reply_to_message_id = message.reply_to_message.message_id
        else:
            self.Voice_object = message.voice
    ...
    def download(self):
        print (self.Voice_object)
        duration = self.Voice_object.duration
        new_file = self.Voice_object.bot.get_file(self.Voice_object.file_id)
...
def on_voice_received(bot, update):
    ...
    if update and update.message:
        VoiceExtended(update.message).download_send()
